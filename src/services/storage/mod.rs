...
impl StorageManager {
    ...
    pub async fn init(&self) -> Result<()> {
        ...
        info!(
            "Storage manager initialized: uploads={:?}, models={:?}",
    ...
    pub async fn preallocate(&self, upload_id: &str, total_size: u64) -> Result<()> {
        ...
        let file = OpenOptions::new()
            ...
            .with_context(|| {
                format!(
                    "Failed to open temp file for preallocation: {:?}",
                    temp_path
                )
            })?;
    ...
    pub async fn append_chunk(
        ...
        file.write_all(data).await.with_context(|| {
            format!("Failed to write chunk {} at offset {}", chunk_index, offset)
        })?;
    ...
    pub async fn finalize(&self, upload_id: &str, filename: &str) -> Result<PathBuf> {
        ...
        fs::rename(&temp_path, &final_path)
            .await
            .with_context(|| format!("Failed to move {:?} to {:?}", temp_path, final_path))?;
