...
use crate::services::storage::{
    StorageManager, DEFAULT_CHUNK_SIZE, MAX_CHUNK_SIZE, MAX_FILE_SIZE, MIN_CHUNK_SIZE,
};
...
impl UploadSessionManager {
    ...
    pub async fn init_upload(
        ...
        info!(
            ...
            upload_id, session.filename, session.total_size, session.total_chunks
        );
    ...
    pub async fn upload_chunk(
        ...
        if session.chunks_received.contains(&chunk_index) {
            warn!(
                "Chunk {} already received for upload {}",
    ...
    pub async fn get_expired_sessions(&self) -> Vec<String> {
        ...
        sessions
            .iter()
            .filter(|(_, session)| {
                session.status == UploadStatus::Uploading && session.created_at < expiry_threshold
            })
            .map(|(id, _)| id.clone())
            .collect()
    }
    ...
    async fn load_sessions_from_db(&self, db: &DatabaseConnection) -> Result<()> {
        ...
        match result {
            Ok(mut response) => {
                ...
                info!(
                    "Loaded {} active upload sessions from database",
                    cache.len()
                );
            }
            Err(e) => {
                warn!(
                    "Could not load sessions from database (table may not exist yet): {}",
                    e
                );
            }
        }
