use fractalmind::models::{EmbeddingModel, EmbeddingVector};
use fractalmind::services::ingestion::{IngestionInput, IngestionService};

#[tokio::test]
async fn test_ingest_service_from_text_file() {
    // Load fixture
    let data = tokio::fs::read("tests/fixtures/samples/sample.txt")
        .await
        .unwrap();
    ...
    let embedding_gen = |_: &str| -> EmbeddingVector {
        EmbeddingVector::new(
            vec![0.01f32; EmbeddingModel::NomicEmbedTextV15.dimension()],
            EmbeddingModel::NomicEmbedTextV15,
        )
    };

    let result = service
        .ingest(input, embedding_gen)
        .await
    ...
    assert!(
        result.node_count() >= 1,
        "expected at least 1 generated node"
    );
...
fn test_ingest_text_direct() {
    ...
    let embedding_gen = |_: &str| -> EmbeddingVector {
        EmbeddingVector::new(
            vec![0.1f32; EmbeddingModel::NomicEmbedTextV15.dimension()],
            EmbeddingModel::NomicEmbedTextV15,
        )
    };

    let res = service.ingest_text(
        text,
        "ns",
        Some("source"),
        vec!["tag1".to_string()],
